# IMPLEMENTATION-PLAN-CHAT-SPECIALIZATION-v5.md

## 1. Overview

This revision replaces `21-IMPL-PLAN-CHAT-MODE-v4.md`. It incorporates the
v4 critique findings from documents 22 and 23:

1. Add `ComplexHash` to canonical messages to prevent tool-call hash collisions.
2. Fix Top-Down Annotation index math to use backward attribution.
3. Fix the FTS rebuild path to use `COALESCE` instead of the built-in `'rebuild'`
   command, which bypasses triggers and would silently drop non-chat entries.
4. Add an explicit `NULL`-vs-empty-string guardrail for sidecar columns.
5. Clarify turn-index convention (0-based in API, 1-based in UI display).
6. Document that the conversation view shows the client-echoed history, not
   necessarily the original proxy-captured response.
7. Describe the 5-attempt parent search honestly as a deliberate lossy
   heuristic, acceptable for MVP.

All prior decisions not explicitly revised here remain in effect:
...
- Keep FTS working for all endpoints via `COALESCE`-based indexing.
- Render conversation view as `root -> selected entry`, not full descendant
  traversal.
- Preserve visibility of `system` and `developer` messages in conversation view.
- Ignore backward compatibility for existing DB contents; rollout will use a
  fresh DB.
- Add explicit truncation guards.
- Write tests alongside each layer, not as a trailing step.
...
with `parent_prefix_len`, these fields define the backward-attribution ranges
used in thread assembly (§11.3).
...
Conversation view will support only `/v1/chat/completions` in v5.
...
It will not attempt descendant traversal in v5. That avoids branch ambiguity
and keeps semantics stable.

The response includes metadata indicating where the selected entry sits in
the visible branch:
...
**Index convention:** The API uses 0-based indexing for `selected_turn_index`.
Frontends display 1-based values to users (e.g., "turn 7 of 7" when the API
returns `selected_turn_index: 6, total_turns_in_view: 7`).
...
for v5 and should be documented.
...
client truncated or modified a previous assistant response to save tokens, the
conversation view shows the modified version, not the original bytes the proxy
captured. This is intentional — it shows exactly what context the LLM received
for the current turn. Users can click `(Log #N)` links to view the original
proxy-captured response for any historical turn.
...
### 4.1 FTS Rebuild Path

The existing codebase uses the built-in FTS5 `'rebuild'` command:

```sql
INSERT INTO openai_logs_fts(openai_logs_fts) VALUES('rebuild')
```

This command bypasses triggers and reads columns directly from the content
table. With `NULL` sidecar columns, a rebuild would silently drop non-chat
entries from the FTS index because it reads `req_text` and `resp_text`
directly, without `COALESCE`.

**Fix:** Replace the built-in `'rebuild'` with a manual equivalent that applies
`COALESCE`:
...
This must be done in a transaction. Update the rebuild logic in
`internal/database/db.go` accordingly.
...
This is acceptable for v5 and should be stated plainly in deployment notes.
...
4. If the message is structurally complex (has `tool_calls`, `function_call`,
   or non-text content parts), set `IsComplex=true` and compute `ComplexHash`
   as described below.
...
### 7.2 ComplexHash for Collision Resistance

Without `ComplexHash`, every tool-call message would canonicalize to the same
form: `{"role":"assistant","is_complex":true}`. A tool-heavy conversation
(e.g., fetching weather for Boston, then for London) would produce identical
prefix hashes despite different payloads.

**Rule:** When a message is marked complex, compute `ComplexHash` from the raw
bytes that make the message unique:

1. Collect the raw `json.RawMessage` fields that triggered the complex
   classification (`tool_calls`, `function_call`, or non-text content parts).
2. Apply `json.Compact` to each to normalize trivial whitespace differences.
3. Concatenate the compacted bytes in a deterministic order.
4. Compute SHA-256 and store as lowercase hex in `ComplexHash`.

This preserves collision resistance for complex messages without requiring
deep semantic parsing of tool-call payloads.
...
This is a deliberate lossy heuristic. It will not find parents in all cases
(e.g., if a client inserts many messages between turns), but it covers the
common patterns at bounded cost. This is acceptable for MVP.
...
The "latest match wins" policy is acceptable for v5 and should be documented
...
Leave `req_text` and `resp_text` as `nil` (Go `*string` pointer, not empty
string).

**Critical guardrail:** The `LogEntry` model uses `*string` pointers for
`ReqText` and `RespText`. When sidecar text is not generated, these must
remain `nil`, never set to `""`. An empty string would cause
`COALESCE(req_text, req_body)` to evaluate to `""`, silently erasing that
entry from the FTS index. This invariant should be enforced in the writer and
covered by a test.

The `COALESCE`-based FTS trigger (§4) ensures that `nil`/`NULL` sidecar
columns fall back to the raw body for indexing. This avoids duplicating raw
bodies into sidecar columns for endpoints we do not understand deeply.
...
5. Set `req_text` to the extracted text (pointer to non-empty string).

If parsing fails, leave `req_text` as `nil` (falls back to raw body in FTS).

### 8.3 Chat/Completions Response Text

For `/v1/chat/completions`, use `ExtractAssistantResponse`:

1. If assistant text is available, set `resp_text` to that plain text (pointer
   to non-empty string).
2. If the response is complex, unsupported, or truncated, leave `resp_text` as
   `nil` (falls back to raw body in FTS).
...
minimum acceptable v5 scope is:

1. Fully improved chat/completions extraction.
2. `nil` sidecar (raw fallback) preserved for all other endpoints.
...
2. On parse failure, log at `WARN` or `DEBUG` and keep sidecar columns `nil`
...
### 11.3 Assembly Algorithm — Top-Down Annotation with Backward Attribution

**This section replaces the v3/v4 assembly algorithm to fix the double-echo bug
and the index-math safety issue.**

A chat completion request already contains the entire conversation history. We
do not need to stitch it together from past requests. We only need the lineage
chain to attribute historical messages to their originating log entries.

**Algorithm:**

1. Fetch the ancestor chain via the CTE (`root -> selected`), yielding an
   ordered list `[E0, E1, ..., En]` where `E0` is the root and `En` is the
   selected entry.
2. Parse the **selected (final) entry's** `req_body.messages`. This is the
   canonical conversation history up to the current turn. Let
   `M = len(messages)`.
3. Attribute messages to log entries using **backward attribution** (see below).
4. Append the **selected (final) entry's** extracted assistant response (from
   `ExtractAssistantResponse`) as the last message in the thread, attributed
   to `En`.

**Backward attribution:**

Attribution walks the chain from `En` backward to `E0`, assigning ownership of
message index ranges in the selected entry's message array. This guarantees
exact coverage with no overlaps and no out-of-bounds panics, even when the
client has rewritten history between turns.

```
cursor = M   (initially: end of message array)
...
- `En` owns `[En.parent_prefix_len, M)`
- `En-1` owns `[En-1.parent_prefix_len, En.parent_prefix_len)`
- ...
- `E0` owns `[0, E1.parent_prefix_len)`

Each message in a range is annotated with the owning entry's `log_id` and
`timestamp`.

**Boundary clamping:** If any `parent_prefix_len` value exceeds `cursor` or is
negative (due to data corruption or client manipulation), clamp it to `cursor`.
This ensures the algorithm never panics and degrades gracefully by attributing
the affected range to the next entry in the chain.
...
for any entry in the chain, treat it as `0` for attribution purposes (meaning
that entry "owns" all messages from the start of its range).
...
- Backward walk guarantees exact array coverage with no gaps or overlaps.
...
3. Render roles distinctly:
   ...
   - `system`
   - `developer`
4. Keep `(Log #N)` clickable back to raw detail view. Include a note in the
   UI that clicking shows the original proxy-captured response, which may
   differ from the client-echoed version displayed in the conversation.
5. Show a header like `Conversation to Log #42 (turn 7 of 7)` (1-based
   display; API value + 1).
...
5. Style `system` and `developer` as inset note blocks rather than chat
   bubbles.
6. On `(Log #N)` links, provide a tooltip or subtle label indicating the link
   shows the original proxy-captured response.
...
5. Show the selected position in the title (1-based display).
...
1. `internal/chat/hash_test.go`
   ...
   - complex marker behavior with `ComplexHash`
   - two different tool-call messages produce different hashes
...
1. `internal/database/writer_test.go`
   - `nil` sidecar for non-chat endpoints (no duplication)
   - populated sidecar for chat/completions
   - empty-string guardrail: verify `""` is never written to `req_text` or
     `resp_text` (always `nil` or non-empty)
...
2. `internal/management/server_test.go`
   ...
   - backward attribution produces correct index ranges
   - partial thread output on mixed simple/complex messages
3. `internal/database/db_test.go`
   - FTS rebuild uses `COALESCE` (non-chat rows remain searchable after
     rebuild)
...
2. sidecar text populated only for chat endpoints (`nil` for others)
3. FTS matches assembled streamed text
4. FTS still works for non-chat endpoints via `COALESCE` fallback
5. FTS still works after manual rebuild for non-chat endpoints
6. thread endpoint returns root->selected in correct order
7. no double-echo in assembled conversation
...
This does not require new metrics for v5, but debug logs should make lineage
...
1. Schema and model changes (including FTS rebuild fix).
2. Chat extraction helpers (request parsing, response extraction) + unit tests.
3. Canonical hashing (canonicalization with `ComplexHash`, SHA-256, prefix
   search) + unit tests.
4. Writer-side sidecar generation and lineage storage + writer tests.
5. Reader thread query + reader tests.
6. Thread API endpoint, DTOs, and backward-attribution assembly + API tests.
...
3. FTS remains correct after a full index rebuild (non-chat entries are not
   silently dropped).
4. A non-stream `/v1/chat/completions` response appears correctly in
   conversation view.
5. `system` and `developer` messages are visible in conversation view.
6. Conversation view for a selected log shows the root-to-selected chain in
   order, with no double-echo of assistant messages.
7. Complex messages are not silently dropped; they are represented with
   placeholders and raw-log links.
8. Different tool-call messages produce different canonical hashes (no
   `IsComplex`-only collision).
9. Raw request/response bodies remain unchanged and accessible.
10. Truncated entries are handled gracefully (no parse attempts, no conversation
    button, raw fallback for FTS).
11. Thread endpoint returns 400 for non-chat and truncated entries, and a valid
    1-turn thread for orphan roots.
12. Sidecar columns never contain empty strings; they are `NULL` or non-empty.
